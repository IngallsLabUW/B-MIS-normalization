...
wArea<- mydata %>% 
  ...
  as.data.frame
wArea.2 <- left_join(wArea, sample.key[,c("Sample.Name","Sample.Group","Bio.Normalization")], by = c("Replicate.Name"="Sample.Name")) %>%
...
rsd.stats <- no.blank.dat %>% select(-Replicate.Name, -Retention.Time,
                                     ...
                                     -runDate, -replicate, -Area, -Area.BlkSub) %>%
